...
class TestLegacy(TestCaseBase):
    ...
    def test_download_t(self):
        import_fakedata()
        with self.app.test_client() as c:
            response = c.get('/download?t=1', follow_redirects=True)
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.charset, 'utf-8')
            self.assertEqual(response.mimetype, 'text/plain')
            data = response.get_data().decode('utf-8')
    ...
    def test_download_j(self):
        import_fakedata()
        with self.app.test_client() as c:
            response = c.get('/download?j=1', follow_redirects=True)
            self.assertEqual(response.status_code, 200)
            self.assertEqual(response.charset, 'utf-8')
    ...
    def test_download_device_id(self):
        import_fakedata()
        with self.app.test_client() as c:
            response = c.get('/download?device_id=DD1234,ABCDEF', follow_redirects=True)
            data = response.get_data().decode('utf-8')
    ...
    def test_download_till_id(self):
        import_fakedata()
        with self.app.test_client() as c:
            response = c.get('/download?till_id=DD1234', follow_redirects=True)
            data = response.get_data().decode('utf-8')
    ...
    def test_download_registration(self):
        import_fakedata()
        with self.app.test_client() as c:
            response = c.get('/download?registration=D-4711,D-OTTO', follow_redirects=True)
            data = response.get_data().decode('utf-8')
    ...
    def test_download_fln(self):
        import_fakedata()
        with self.app.test_client() as c:
            response = c.get('/download/download-fln.php', follow_redirects=True)
            data = response.get_data().decode('utf-8')
